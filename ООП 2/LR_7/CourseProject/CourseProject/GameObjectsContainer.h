...
public:
	...
	std::vector<GameObject*> to_destroy_;
	...
	void update(sf::RenderWindow* window)
	{
		...
		for (auto element : to_destroy_)
		{
			objects_.erase(std::remove(objects_.begin(), objects_.end(), element), objects_.end());

			element->transform->set_parent(nullptr);
			element->on_destroy();
	...
	std::vector<GameObject*> check_for_collision(GameObject* gameObject) const
	{
		Graphic* graphic = gameObject->get_component<Graphic>();
