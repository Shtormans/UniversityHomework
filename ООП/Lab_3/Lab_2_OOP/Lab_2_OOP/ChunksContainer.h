...
public:
	...
	HeapChunk* GetByIndex(int index) const
	{
		HeapChunk* current = _start;
		int i = 0;
	...
	void DebugMemory(std::ostream& out) const
	{
		HeapChunk* current = _start;
		int i = 0;
		...
		{
			out << "Item: " << i << '\n';
			out << "In Use: " << (current->InUse ? "true" : "false") << '\n';
			out << "Size: " << current->Size << '\n';
			out << "Unoccupied size: " << current->GetUnoccupiedMemorySize() << '\n';
			out << "Functional size: " << HeapChunk::GetFunctionalMemoryUsage() << '\n';
			out << '\n';
